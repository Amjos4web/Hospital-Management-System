	  ...
	  if (strlen($_POST['homeAddress']) < 50)
	  {
		  if (preg_match("/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/", $Dob))
		  {
			  if (preg_match("/^[0-9]{11}$/", $phoneNo))
				  {
					$sql = "UPDATE `som` SET `sex`='$gender', `marital`='$marital', `maidenName`='$maidenName', `DateofBirth`='$Dob', `natioNality`='$nationality', `stateOforigin`='$stateOforigin', `homeTown`='$homeTown', `LGA`='$LGA', `religion`='$religion', `phoneNo`='$phoneNo', `homeAddress`='$homeAddress', `denoMination`='$denoMination', `postaladdress`='$postal' WHERE `eMail`='$email'";
					$check = mysqli_query($dbconnect, $sql);
...
<title>Bio-Data/School of Midwifery</title>
</head>
...
<div id="container">
  <div id="sidebar1"><br>
	...
	<img src="images/bowen1.jpg" width="170px" height="250px" style="margin-left: 20px" alt="Welcome To School of Midwifery"><br><br>
