...
if (isset($_POST['submit'])) {
	...
	if ((empty($pin && $surName && $firstName && $otherName && $passWord && $confirmPass && $regno) == false) && ($passWord) == ($confirmPass))
	{
		include "dbconnect.php";     
		$sql = "SELECT * FROM `som` WHERE `pin`='$pin'";	 
		...
		if ($numrow == 1)
		{
			...
			if ($Email == true)
			{
				$query = "UPDATE `som` SET `registeredNo`='$regno', `surName`='$surName', `firstName`='$firstName', `otherName`='$otherName',  `passWord`='$passWord', `confirmPass`='$confirmPass', `date_added`='$date_added', `year`='$year', `studentid`='$studentId' WHERE `pin`='$pin'";
				mysqli_query($dbconnect, $query);
				$max = "SELECT  MAX(`studentid`) AS student_id FROM `som`";
				$max2 = mysqli_query($dbconnect, $max);
				$result1 = mysqli_fetch_array($max2);
				$result2 = $result1['student_id'];
				$result3 = $result2+1;
				$sql = "UPDATE `som` SET `studentid`='$result3' WHERE `pin`='$pin'";	
				$result4 = mysqli_query($dbconnect, $sql);					 
				$qery = "SELECT `id` as idmax FROM `som`";
				$result = mysqli_query($dbconnect, $qery) or die (mysqli_error($dbconnect));
				$rowa = mysqli_fetch_array($result);
				$rw = $rowa['idmax'];
				$rwo = $rw+1;
				$rwo = str_pad($rwo,4,'0',STR_PAD_LEFT);
				$sql = "UPDATE `som` SET `id`='$rwo' WHERE `pin`='$pin'";
  ...
  <div id="sidebar1"><br>
    ...
    <ul id="navigation2">
      ...
      <li><a href="login.php">Login</a></li><br>
		 ...
		 <img src="images/bowen2.jpg" width="170px" height="250px" style="margin-left: 20px" alt="Welcome To School of Midwifery"><br><br>
		...
		<div class="reg_form">
		    <form action="" method="post"> 
			 ...
			 <?php
			  ...
			  <?php echo $msg; ?>
			  <tr>
			   <td width="30%"><label>Nursing Registered No</label></td>
			   <td width="70%"><input type="text" class="userarea_reg" name="regNo" autofocus></td>
			  </tr>
			  <tr>
