...
if (isset($_POST['searchbtn'])){
	if (isset($_POST['search2']) && $_POST['search2'] != ""){
		...
		if ($count > 0){
			while($row2=mysqli_fetch_assoc($query)){
				...
				$dynamic_list .= "<tr>";
				$dynamic_list .= "<td style='background-color:#CECECE; text-align: center; font-family: Adobe Heiti Std R;'>".$staff_id . "</td>";
				$dynamic_list .= "<td style='background-color:#CECECE; text-align: center; font-family: Adobe Heiti Std R;'>".$basic . "</td>";
				$dynamic_list .= "<td style='background-color:#CECECE; text-align: center; font-family: Adobe Heiti Std R;'>" .$housing . "</td>";
				$dynamic_list .= "<td style='background-color:#CECECE; text-align: center; font-family: Adobe Heiti Std R;'>".$trans . "</td>";
				$dynamic_list .= "<td style='background-color:#CECECE; text-align: center; font-family: Adobe Heiti Std R;'>".$date . "</td>";
				$dynamic_list .= "<td style='background-color:#CECECE; text-align: center; font-family: Adobe Heiti Std R;'>" .$per . "</td>";
				$dynamic_list .= "<td style='background-color:#CECECE; text-align: center; font-family: Adobe Heiti Std R;'>".$per_total . "</td>";
				$dynamic_list .= "<td style='background-color:#CECECE; text-align: center; font-family: Adobe Heiti Std R;'>".$total . "</td>";
				$dynamic_list .= "</tr>";
			...
			windows.locaton = 'individual_pension.php';
			</script><?php
		}
	} else {
		?><script type="text/javascript">
	...
	// total sum
	$sum = "SELECT  SUM(`total`) AS total_sum1 FROM `pension_form` WHERE `staff_id`='$search' AND `date` LIKE '%$choose_year%'";
	...
	// percentage sum
	$sum = "SELECT  SUM(`per_total`) AS total_sum2 FROM `pension_form` WHERE `staff_id`='$search' AND `date` LIKE '%$choose_year%'";
	$sum2 = mysqli_query($dbconnect, $sum) or die (mysqli_error($dbconnect));
	$result1 = mysqli_fetch_array($sum2);
...
<title>Individual Pension List</title>
</head>
<body>
<?php
     ...
     <div class="margin" id="content">
	   ...
	   <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
		<div class="search_angle">
			<center><h3 class="heading_text">Individual pension list</h3></center><br>
			...
			<center><input type="submit" value="Search & Sum" name="searchbtn" class="submit4"></center>
		 </form><br>
	   ...
	   <tr>
		...
		<!--tr>
		 ...
		 <td>&nbsp;</td>
			...
			<center><h3 class="heading_text">Total For <?php if (isset($_POST['searchbtn'])){ echo $staff_id . " " . "in" . " " . $choose_year;} ?></h3></center>
			...
			<center><h3 class="heading_text">Percentage Total For <?php if (isset($_POST['searchbtn'])){ echo $staff_id . " " . "in" . " " . $choose_year;} ?></h3></center>
		...
		<form action="individual_pensionRec.php?staffId=$staff_id&year=$choose_year" method="GET" id="jsform" target="_blank">
