...
if (isset($_POST['login'])){
	...
	$last_log_date = date('Y-m-d H:i:s');
		if (empty($email && $password) == false){
			$sql = "SELECT * FROM `ipd_billings` WHERE `emailAdd`='$email' && `passWord`='$password'";
			$check = mysqli_query($dbconnect, $sql) or die (mysqli_error($dbconnect));
			$result = mysqli_num_rows($check);
			if ($result > 0){
				session_start();
				$_SESSION['emaill'] = $email;
				header('location: ipd_billings_home.php');
				$sql = "UPDATE `ipd_billings` SET `last_log_date`='$last_log_date' WHERE `emailAdd`='$email'";
...
<title>IPD Billings Login Page</title>
</head>
<body>
<?php
    ...
    <p class="subHeader">Menu</p>
     ...
     <div class="margin" id="content">
	  <div class="login_form">
	   <fieldset>
		 <legend>IPD Billings Login Section</legend>
