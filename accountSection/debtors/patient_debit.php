...
if (isset($_POST['searchbtn'])){
	if (isset($_POST['patient_id']) && $_POST['patient_id'] != ""){
		...
		$sql2 = "SELECT * FROM `debit_ledger` WHERE `staff_id`='".$patient_id."' ORDER BY date DESC";
		...
		if($resultCount2>0){
			while($row=mysqli_fetch_array($check2)){
				...
				$search_results .= '<td style="background-color:#C5DFFA; width: 70%"><input type="text" id="userarea" style="margin-left: 0px; width: 318px"  name="patient_id" value="'.$patient_id.'"></td>';
				$search_results .= "</tr>";
			...
			$search_result .= '<td style="background-color:#C5DFFA; width: 70%"><input type="text" id="userarea" style="margin-left: 0px; width: 318px"  name="patient_id" value="'.$patient_id.'"></td>';
			$search_result .= "</tr>";
			...
			$search_result .= '</form>';
			$not_found = "<center><h2 class='title' style='font-size: 20px; font-style: normal; font-weight: bold; text-shadow: 1px 1px 1px rgba(0,0,0,0.2); color: #880000'>No result found for $patient_id.. Fill the form below</h2></center><br>";
...
if($resultCount2>0){
	while($row=mysqli_fetch_array($check2)){
		...
		// sum for credit
		$creditsum = "SELECT  SUM(`credit`) AS credit_total FROM `debit_ledger` WHERE `staff_id`='".$patient_id."'";
		...
		// sum for debit
		$debitsum = "SELECT  SUM(`debit`) AS debit_total FROM `debit_ledger` WHERE `staff_id`='".$patient_id."'";
		...
		$debits_lists .= "<td style='background-color:#CECECE; text-align: center; font-family: arial;'>" . $patient_id . "</td>";
...
if (isset($_POST['update']))
{
	...
	if (empty($accode && $date && $par && $credit && $debit && $approve) == false){
		$sql5 = "INSERT INTO debit_ledger (staff_id, ac_code, particulars, date, credit, debit, approved_by, balance) VALUES ('$patient_id', '$accode', '$par', '$date', '$credit', '$debit', '$approve', '$balance')";
...
<title>Patient Debtors Ledger</title>
	...
	<?php include_once($_SERVER['DOCUMENT_ROOT'].'../buth_net/new_bar.php'); ?>
     ...
     <div class="margin" id="content">
	  <div class="search_angle">
			<center><h3 class="heading_text">Patient Debtors Ledger</h3></center><br>
			<center><img src="../images/payment2.jpg" alt="search_angle" width="500px" height="200px"></center><br>
			<div class="search">
			<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
			<center><input type="text" name="patient_id" id="search2"  maxlength="20" placeholder="Enter patient id"><br><br></center>
	   ...
	   <table width="700px" class="table1" style="margin-left: auto; margin-right: auto" cellpadding="2" cellspacing="2" border="1">
		<tr>
			...
			<td width="10%" style="background-color:#C5DFFA; font-family: arial black; text-align: center; font-size: 14px"><b>Patient Id</b></td>
