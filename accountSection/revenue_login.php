...
if (isset($_POST['login'])){
	...
	$last_log_date = date('Y-m-d H:i:s');
		if (empty($email && $password) == false){
			$sql = "SELECT * FROM `revenue_login` WHERE `emailAdd`='$email' && `passWord`='$password'";
			...
			if ($result > 0){
				...
				$sql = "UPDATE `revenue_login` SET `last_log_date`='$last_log_date' WHERE `emailAdd`='$email'";
...
<title>Revenue Login Page</title>
</head>
<body>
<?php
    ...
    <p class="subHeader">Menu</p>
     ...
     <div class="margin" id="content">
	  <div class="login_form">
	   <fieldset>
		 <legend>Revenue Login Section</legend>
