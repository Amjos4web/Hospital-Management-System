...
if (isset($_POST['login'])){
	...
	$last_log_date = date('Y-m-d H:i:s');
		if (empty($email && $password) == false){
			$sql = "SELECT * FROM `nhis_form` WHERE `emailAdd`='$email' && `passWord`='$password'";
			$check = mysqli_query($dbconnect, $sql) or die (mysqli_error($dbconnect));
			$result = mysqli_num_rows($check);
			if ($result > 0){
				session_start();
				$_SESSION['emaill'] = $email;
				header('location: nhis_home.php');
				$sql = "UPDATE `nhis_form` SET `last_log_date`='$last_log_date' WHERE `emailAdd`='$email'";
    ...
    <p class="subHeader">Menu</p>
     ...
     <div class="margin" id="content">
	  <div class="login_form">
	   <fieldset>
		 <legend>NHIS Login Section</legend>
