...
if (isset($_POST['login'])){
	...
	$last_log_date = date('Y-m-d H:i:s');
		if (empty($email && $password) == false){
			$sql = "SELECT * FROM `cashier` WHERE `username_id`='$email' && `password`='$password'";
			...
			if ($result > 0){
				...
				$sql = "UPDATE `cashier` SET `last_log_date`='$last_log_date' WHERE `username_id`='$email'";
    ...
    <p class="subHeader">Menu</p>
     ...
     <div class="margin" id="content">
	  <div class="login_form">
	   <fieldset>
		 <legend>Cashier Login Section</legend>
