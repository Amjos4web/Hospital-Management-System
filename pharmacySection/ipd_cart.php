...
if (isset($_POST['id'])){
    ...
    if (!isset($_SESSION['CartArray5']) || count($_SESSION['CartArray5']) < 1){
		...
		$_SESSION['CartArray5'] = array(0 => array('item_id' => $product_id, 'quantity' => 1));
	} else {
		...
		foreach ($_SESSION['CartArray5'] as $each_item){
			...
			while (list($key, $value) = each($each_item)){
				if ($key == 'item_id' && $value == $product_id){
					...
					array_splice($_SESSION['CartArray5'],$i-1,1, array(array('item_id' => $product_id, 'quantity' => $each_item['quantity'] + 1)));
					$wasFound = true;
				} // close the if condition
			} // close the while loop
		} // close the for each loop
	if ($wasFound == false){
		array_push($_SESSION['CartArray5'], array('item_id' => $product_id, 'quantity' => 1));
	}
  }
  header("location: ipd_cart.php");
  exit();
}
?>
<?php
	 ...
	 foreach ($_SESSION['CartArray5'] as $each_item){
		...
		while (list($key, $value) = each($each_item)){
			if ($key == 'item_id' && $value == $item_to_adjust){
				// That item is in cart already, so let's adjust the quantity using array_splice()
				array_splice($_SESSION['CartArray5'],$i-1,1, array(array('item_id' => $item_to_adjust, 'quantity' => $quantity)));
			} // close the if condition
		} // close the while loop
	} // close the for each loop
}       
...
if (isset($_POST['index_to_remove']) && $_POST['index_to_remove'] != ""){
	...
	if (count($_SESSION['CartArray5']) <= 1){
		unset($_SESSION['CartArray5']);
	} else {
		unset($_SESSION['CartArray5'][$key_to_remove]);
		sort($_SESSION['CartArray5']);
	}
...
if(!isset($_SESSION['CartArray5']) || count($_SESSION['CartArray5']) < 1){
	...
	foreach ($_SESSION['CartArray5'] as $each_item){
		...
		$sql2 = "SELECT * FROM `ipd_pharm_req` WHERE `id`='$item_id'";
  ...
  <div class="margin" id="content">
	<div class="product_details">										
	 ...
	 <div class="form_table1" style="display: none">
	   <form action="ipd_receipt.php" method="POST" id="jsform" class="form1" target="_blank">
	   ...
	   <form action="ipd_staff_receipt.php" method="POST" id="jsform2" class="form2"  target="_blank">
	   ...
	   <form action="ipd_sem_receipt.php" method="POST" id="jsform3" class="form3"  target="_blank">
	   ...
	   <form action="ipd_nhis_receipt.php" method="POST" id="jsform4" class="form4" target="_blank">
	   ...
	   <tr>
		...
		<label class="add_more_drugs" style="float: right"><a href='ipd_dashboard.php'>Add More Drugs</a></label>
	   ...
	   <h2 class="empty"><a href='ipd_cart.php?cmd=emptycart'>Click here to empty your cart</a></h2>
          <hr style='width: 800px; margin-left: 0px; height: 6px; background-color: #14BCEB'><br>
	<!-- end .margin --></div>
  <!-- end .container --></div>
 <?php
