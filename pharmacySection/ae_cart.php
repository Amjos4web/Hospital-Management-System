...
if (isset($_POST['id'])){
    ...
    if (!isset($_SESSION['CartArray3']) || count($_SESSION['CartArray3']) < 1){
		...
		$_SESSION['CartArray3'] = array(0 => array('item_id' => $product_id, 'quantity' => 1));
	} else {
		...
		foreach ($_SESSION['CartArray3'] as $each_item){
			...
			while (list($key, $value) = each($each_item)){
				if ($key == 'item_id' && $value == $product_id){
					...
					array_splice($_SESSION['CartArray3'],$i-1,1, array(array('item_id' => $product_id, 'quantity' => $each_item['quantity'] + 1)));
					$wasFound = true;
				} // close the if condition
			} // close the while loop
		} // close the for each loop
	if ($wasFound == false){
		array_push($_SESSION['CartArray3'], array('item_id' => $product_id, 'quantity' => 1));
	}
  }
  header("location: ae_cart.php");
  exit();
}
?>
<?php
	 ...
	 foreach ($_SESSION['CartArray3'] as $each_item){
		...
		while (list($key, $value) = each($each_item)){
			if ($key == 'item_id' && $value == $item_to_adjust){
				// That item is in cart already, so let's adjust the quantity using array_splice()
				array_splice($_SESSION['CartArray3'],$i-1,1, array(array('item_id' => $item_to_adjust, 'quantity' => $quantity)));
			} // close the if condition
		} // close the while loop
	} // close the for each loop
}
...
if (isset($_POST['index_to_remove']) && $_POST['index_to_remove'] != ""){
	...
	if (count($_SESSION['CartArray3']) <= 1){
		unset($_SESSION['CartArray3']);
	} else {
		unset($_SESSION['CartArray3'][$key_to_remove]);
		sort($_SESSION['CartArray3']);
	}
...
<div id="container">
  <div id="sidebar1"><br>
    ...
    <ul id="navigation2">
	  ...
	  <li><a href="ae_pharm_dashboard.php">A and E Pharmacy</a></li><br>
	...
	<div class="product_details">
	 ...
	 <h2 style='text-align: center; font-family: Calibri (Body); font-size: 22px; text-transform: uppercase; font-style: normal; color: #880000; text-shadow: 0 1px 0 #ccc,0 2px 0 #c9c9c9,0 3px 0 #bbb,0 4px 0 #b9b9b9,0 5px 0 #aaa,0 6px 1px rgba(0,0,0,.1),0 0 5px rgba(0,0,0,.1),0 1px 3px rgba(0,0,0,.3),0 3px 5px rgba(0,0,0,.2),0 5px 10px rgba(0,0,0,.25),0 10px 10px rgba(0,0,0,.2),0 20px 20px rgba(0,0,0,.15); font-style: normal'>Your Cart</h2>
	  ...
	  <div class="form_table1" style="display: none">
	   <form action="ae_receipt.php" method="POST" id="jsform" class="form1" target="_blank">
	   ...
	   <form action="ae_staff_receipt.php" method="POST" id="jsform2" class="form2"  target="_blank">
	   ...
	   <form action="ae_sem_receipt.php" method="POST" id="jsform3" class="form3"  target="_blank">
	   ...
	   <form action="ae_nhis_receipt.php" method="POST" id="jsform4" class="form4" target="_blank">
	   ...
	   <tr>
		...
		<td width="12%" style='background-color:#C5DFFA; font-family: arial black; font-size: 14px'><b>Quantity Available</b></td>
		...
		<label class="add_more_drugs" style="float: right"><a href='ae_pharm_dashboard.php'>Add More Drugs</a></label>
	   ...
	   <h2 class="empty"><a href='ae_cart.php?cmd=emptycart'>Click here to empty your cart</a></h2>
          <hr style='width: 800px; margin-left: 0px; height: 6px; background-color: #14BCEB'><br>
	<!-- end .margin --></div>
  <!-- end .container --></div>
 <?php
