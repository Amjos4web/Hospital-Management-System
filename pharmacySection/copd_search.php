...
if (isset($_POST['searchbtn'])){
	if (isset($_POST['search']) && $_POST['search'] != ""){
		...
		$sqlcommand = "SELECT `id`, `product_name`, `category`, `unit_price`, `drug_type`, `date_added`, `quantity` AS name FROM `copd_pharm_req` WHERE `product_name` LIKE '%$search%' OR `category` LIKE '%$search%'";
		...
		if ($count > 0){
			while($row2=mysqli_fetch_array($query)){
				...
				$search_results .= "<td style='background-color:#CECECE; font-family: Adobe Heiti Std R;padding-left: 5px; width: 15%'>" . "<a href='copd_products.php?product_id=$product_id' style='font-style: normal; font-family: Adobe Heiti Std R; color: #880000'>View Details</a></td>";
				$search_results .= "</tr>"; 
