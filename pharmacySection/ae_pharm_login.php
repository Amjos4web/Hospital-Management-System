...
if (isset($_POST['login'])){
	...
	if (empty($email && $password) == false){
		$sql = "SELECT * FROM `ae_pharm` WHERE `ae_staffEmail`='$email' && `ae_staffPass`='$password'";
		$check = mysqli_query($dbconnect, $sql) or die (mysqli_error($dbconnect));
		$result = mysqli_num_rows($check);
		if ($result > 0){
			session_start();
			$_SESSION['emaill'] = $email;
			header('location: ae_pharm_dashboard.php');
			$sql = "UPDATE `ae_pharm` SET `last_log_date`='$last_log_date' WHERE `ae_staffEmail`='$email'";
...
<title>A and E PharmacyLogin Page</title>
</head>
<body>
<?php
    ...
    <ul id="navigation2">
     ...
     <div class="margin" id="content">
	  <div class="login_form">
	   <fieldset>
		 <legend>A and E Pharmacy Login Section</legend>
