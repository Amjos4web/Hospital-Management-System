...
$sem_name = preg_replace("#[^a-z.]#i", "",$_POST['sem_name']);
$sem_id = $_POST['sem_id'];
$payment_date = date('Y-m-d');
$voucher_paid_for = "Drugs";


if (empty($sem_name && $sem_id) == false)
{
	$sql7 = "SELECT * FROM `transactions` WHERE `hospital_no`='$sem_id'";
	$query7 = mysqli_query($dbconnect, $sql7) or die (mysqli_error($dbconnect));
	$check8 = mysqli_num_rows($query7);
	
	$sql8 = "INSERT INTO `transactions` (`patient_name`,`hospital_no`,`total_amount`,`total_amount2`,`payment_date`,`staff_id`, `paid_for`) VALUES ('$sem_name', '$sem_id', '$cartTotal', '$per', '$payment_date', '$name', '$voucher_paid_for')";
	...
	?><!doctype html>
	 ...
	 <head>
	 <meta charset="utf-8">
	 <meta http-equiv="refresh" content="5;url=http://localhost/buth_net/pharmacySection/mopd_sem_receipts.php/" />
	...
	?><script type="text/javascript">
	  alert ('Please enter the seminarian name and ID');
	  window.location = "mopd_cart.php";
	  </script><?php 
