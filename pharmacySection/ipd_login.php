...
if (isset($_POST['login'])){
	...
	$last_log_date = date('Y-m-d H:i:s');
		if (empty($email && $password) == false){
			$sql = "SELECT * FROM `ipd_login` WHERE `emailAdd`='$email' && `passWord`='$password'";
			$check = mysqli_query($dbconnect, $sql) or die (mysqli_error($dbconnect));
			$result = mysqli_num_rows($check);
			if ($result > 0){
				session_start();
				$_SESSION['emaill'] = $email;
				header('location: ipd_dashboard.php');
				$sql = "UPDATE `ipd_login` SET `last_log_date`='$last_log_date' WHERE `emailAdd`='$email'";
...
<title>IPD Pharmacy Login Page</title>
</head>
<body>
<?php
    ...
    <ul id="navigation2">
     ...
     <div class="margin" id="content">
	  <div class="login_form">
	   <fieldset>
		 <legend>IPD Pharmacy Login Section</legend>
