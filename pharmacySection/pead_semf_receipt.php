...
$cartTotal = "";
	if(!isset($_SESSION['CartArray4']) || count($_SESSION['CartArray4']) < 1){
		...
		foreach ($_SESSION['CartArray4'] as $each_item){
			...
			$sql2 = "SELECT * FROM `pead_pharm_req` WHERE `product_id`='$item_id' LIMIT 1";
...
$hospital_no = preg_replace("#[^0-9]#i", "",$_POST['hospital_no']);
...
if (empty($patient_name && $hospital_no) == false)
{
	...
	?><!doctype html>
	 ...
	 <head>
	 <meta charset="utf-8">
	 <meta http-equiv="refresh" content="5;url=http://localhost/buth_net/pharmacySection/pead_receipts.php/" />
