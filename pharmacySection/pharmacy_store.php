...
if (isset($_POST['login'])){
	...
	$last_log_date = date('Y-m-d H:i:s');
		if (empty($email && $password) == false){
			$sql = "SELECT * FROM `pharmacy_store` WHERE `emailAdd`='$email' && `passWord`='$password'";
			$check = mysqli_query($dbconnect, $sql) or die (mysqli_error($dbconnect));
			$result = mysqli_num_rows($check);
			if ($result > 0){
				session_start();
				$_SESSION['emaill'] = $email;
				header('location: pharmacy_store_dashboard.php');
				$sql = "UPDATE `pharmacy_store` SET `last_log_date`='$last_log_date' WHERE `emailAdd`='$email'";
...
<title>Pharmacy Store Login Page</title>
</head>
<body>
<?php
    ...
    <ul id="navigation2">
     ...
     <div class="margin" id="content">
	  <div class="login_form">
	   <fieldset>
		 <legend>Pharmacy Store Login Section</legend>
