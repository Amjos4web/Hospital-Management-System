...
if($resultCount8>0){
	...
	$id=$row["id"];
	$nhis_name=$row["patient_name"];
	$nhis_id=$row["hospital_no"];
		...
		$cartOutput = "<p style='color: red; text-align: center; font-weight: bold; font-size: 20px'>Your Billing Cart is empty</p>";
			...
			foreach ($_SESSION['CartArray5'] as $each_item){
				...
				while ($row2=mysqli_fetch_array($check2)){
					...
					$sql4 = "SELECT * FROM `ipd_pharm_req` WHERE `id`='$item_id' LIMIT 1";
					$check4 = mysqli_query($dbconnect, $sql4);
					while ($row2=mysqli_fetch_array($check4)){
						$quantity = $row2['quantity'];
					}
					$queryLan = "SELECT * FROM `ipd_pharm_req` WHERE `id`='$item_id' & `quantity`='$quantity'";
					...
					$upDate = "UPDATE `ipd_pharm_req` SET `quantity`='$newQuantity' WHERE `id`='$item_id'"; 
					$Check2 = mysqli_query($dbconnect, $upDate) or die (mysqli_error($dbconnect));
					
					$queryLan2 = "SELECT * FROM `ipd_pharm_req` WHERE `id`='$item_id' & `quantity`='$quantity'";
					$Check = mysqli_query($dbconnect, $queryLan2) or die (mysqli_error($dbconnect));
					$emptyQuantity = "0";
					if ($newQuantity < $emptyQuantity){
						$null = "-";
						$upDate = "UPDATE `ipd_pharm_req` SET `quantity`='$null' WHERE `id`='$item_id'";
						$Check3 = mysqli_query($dbconnect, $upDate) or die (mysqli_error($dbconnect));
						$upDate5 = "DELETE FROM `ipd_pharm_req` WHERE `quantity`='$null' && `id`='$item_id'";
...
<div id="receipt_wrapper" style="width: 272px; height: 756px; margin-left: auto; margin-right: auto">
  ...
  <p style="text-align: center; font-family: Calibri (Body); font-weight: bold; font-size: 14px">NHIS Drug Charge Slip</p>
  <label style="font-family: Calibri (Body); font-weight: bold; font-size: 13px"><?php echo "Payer ID:" . " " .$nhis_name; ?></label>
  <label style="float: right; font-family: Calibri (Body); font-weight: bold; font-size: 14px"><?php echo "Hosp. No:" . " " .$nhis_id; ?></label>
  ...
  <label style="float: left; font-family: Calibri (Body); font-weight: bold; font-size: 14px"><?php echo "Being Payment For:" . " " .$voucher_paid_for; ?></label><br>
	 <table width="272px"  style="margin-left: auto; margin-right: auto; min-height: 150px" cellspacing="0" border="0" >
	  ...
	  <?php echo '<h2 style="font-family: arial black;text-align: center; font-weight: bold; font-size: 15px">90% Off: N'.$cartTotal.'</h2>' ?>
