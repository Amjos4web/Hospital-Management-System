...
if (isset($_POST['searchbtn'])){
	if (isset($_POST['search']) && $_POST['search'] != ""){
		...
		$sqlcommand = "SELECT `product_id`, `product_name`, `category`,  `storeType`, `cost_price`, `date_added`, `quantity_available`, `expiring_date` AS name FROM `products` WHERE `product_name` LIKE '%$search%' OR `category` LIKE '%$search%'";
		...
		if ($count > 0){
			while($row2=mysqli_fetch_array($query)){
				...
				$product_price=$row2["cost_price"];
				$date_added=$row2["date_added"];
				$storeType=$row2["storeType"];
				...
				$search_results .= "<td style='background-color:#CECECE; font-family: Adobe Heiti Std R; padding-left: 5px; width: 40%'>" . $storeType . "</td>";
				...
				$search_results .= "<td style='background-color:#CECECE; font-family: Adobe Heiti Std R;padding-left: 5px; width: 15%'>" . "<a href='pharmacy_products.php?product_id=$product_id' style='font-style: normal; font-family: Adobe Heiti Std R; color: #880000'>View Details</a></td>";
				$search_results .= "</tr>"; 
     ...
     <div class="margin" id="content">
	  <div class="welcome_message">
	   <h1 style='text-align: center; font-family: tahoma; font-size: 16px; text-transform: uppercase; font-weight: bold; background-color: #000000; color: #CECECE'>Welcome <?php echo $name."!"; ?> What would you like to do today?</h1>
		 ...
		 <form action="pharmacy_search.php" method="post">
