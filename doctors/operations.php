...
if (isset($_GET['hospital_no'])){
	...
	if (isset($_POST['submit'])){
		...
		if (empty($date && $codeNo && $operation) == false) 
		{
			if ((preg_match("/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/", $date)))
			{
			  $sql = "INSERT INTO operations (`logged_in_staff`, `server_date`, `date`, `operation`, `code_number`, `hosp_no`) VALUES ('$name', '$server_date', '$date', '$operation', '$codeNo', '$hospital_no')";
			  ...
			  header ("refresh: 3; url=operations.php?hospital_no=".$hospital_no."");
			} else
...
$selectview = "SELECT * FROM operations WHERE hosp_no='".$hospital_no."'";
...
if ($checkresult > 0){
	while($row=mysqli_fetch_array($checkview)){
		...
		$hospital_data .= "<td class='data_td'>" . $ope . "</td>";
		$hospital_data .= "<td class='data_td'>" . $code . "</td>";
		$hospital_data .= "<td class='data_td'>" . $added_by . "</td>";
		$hospital_data .= "</tr>"; 
	}
} else
	$nodata = "<p style='color: #D8000C; background-color: #FFBABA; border-radius:.5em; width: 300px; border: 1px solid #D8D8D8; padding: 5px; border-radius: 5px; margin-left: auto; margin-right: auto; font-family: Arial; font-size: 11px; text-transform: uppercase; text-align: center; text-transform: uppercase'>No Operations Data</p>";
?>
<!doctype html>
<html>
     ...
     <div class="margin" id="content">
	  ...
	  <div class="bio_data">
		<div class="rec_new_form">
			 ...
			 <div class="form_data">
			 <center><h3 class="heading_text">Patient Operations Data</h3></center>
			 <?php echo $msg; ?>
			 ...
			 <tr>
			  ...
			  <tr>
			   <td width="30%"><label>Operation</label></td>
			   <td width="70%"><input type="text" id="userarea" name="operation" value= "<?=@$operation?>"></td>
			  </tr>
			  <tr>
	   ...
	   <table cellpadding="2" cellspacing="2">
		   <tr>
			  ...
			  <td style="width: 440px; text-align: center;">Operations</td>
