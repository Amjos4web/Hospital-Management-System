...
if (isset($_POST['login'])){
	...
	$email = trim($_POST['emaill']);
		if (empty($email && $password) == false){
			$sql = "SELECT * FROM `record_staff` WHERE `rec_staff_email`='$email' && `rec_staff_pass`='$password'";
			$check = mysqli_query($dbconnect, $sql) or die (mysqli_error($dbconnect));
			$result = mysqli_num_rows($check);
			if ($result > 0){
				session_start();
				$_SESSION['emaill'] = $email;
				header('location: rec_staff_dashboard.php');
				} else
...
<div id="container">
  <div id="sidebar1"><br>
    ...
    <ul id="navigation2">
	  ...
	  <li><a href="#">Admin Login Page</a></li><br>
	...
	<?php include_once "../new_bar.php"; ?>
     ...
     <div class="margin" id="content">
	  <div class="login_form">
	   <fieldset>
		 <legend>Staff Login Section</legend>
